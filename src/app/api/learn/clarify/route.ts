...
export async function POST(req: Request) {
    ...
    const { lessonId, quote, question, threadId, blockIndex, sectionIndex, lessonContext } = body;
    if (!lessonId || !quote || !question || !threadId || typeof blockIndex !== "number" || typeof sectionIndex !== "number") {
    ...
    try {
        ...
        // Get thread history
        const allMessages = await convex.query(api.courseLessonMessages.getForLesson, {
            lessonId: lessonIdTyped,
        });
        const threadMessages = allMessages.filter((m) => m.threadId === threadId);
        const historyStr = threadMessages
            .slice(-10)
            .map((m) => `${m.role === "teacher" || m.role === "system" ? "AI" : "Student"}: ${m.content}`)
            .join("\n");

        // Build prompt with full context
        const promptDoc = await convex.query(api.coursePrompts.getPrompt, { name: "clarifier" });
